...
OpenClaw should use a layered memory architecture: FeedSilo as the raw capture
...
- FeedSilo stores raw captured material: tweets, articles, threads, media,
  source URLs, summaries, and extracted text.
- The FeedSilo Memory Gateway exposes that captured text directly to Codex and
  OpenClaw agents.
...
- The Legion embedding sidecar provides the shared 1536-dimensional embedding
  path for both FeedSilo agent memory and GBrain.

This division avoids turning GBrain into a duplicate archive. FeedSilo answers
"what did we save?" while GBrain answers "what do we now know?"

## Architecture Principles

1. **Raw capture and synthesized memory should stay separate.** FeedSilo keeps
...
5. **The endgame is a shared memory surface.** Agents should not need to know
   whether useful context lives in FeedSilo, GBrain, email, calendar, or a future
...
- Garry Tan introduced GBrain as an MIT-licensed OpenClaw/Hermes brain that can
  ...
  [Source: FeedSilo item b3ee9ebe-581c-429c-9acc-7c24289afbd6,
...
- Vox summarized the GBrain/OpenClaw pattern: `RESOLVER.md`, `SOUL.md`,
  ...
  [Source: FeedSilo item b37b5210-6da6-443f-9f4a-f443b2b34d96,
...
- OpenClaw Active Memory is the missing recall loop: storage, historical
  ...
  [Source: FeedSilo item 5028d46a-88dd-429c-bfb1-d87c96ec1142,
...
- Abbie Tyrell's OpenClaw memory thread frames the production lifecycle as
  ...
  [Source: FeedSilo item f126a091-876a-4d73-8bfc-1f4646bb601b,
...
- The Karpathy/Graphify cluster argues that agents should build persistent
  ...
  [Source: FeedSilo item dd6547ff-a2d0-4d1f-92f4-591be45a60da,
...
- The relationship-graph thread says the real leverage is not a flat pile of
  ...
  relationships.
  [Source: FeedSilo item 67acdde4-7cd1-4683-9ff0-22de460de2b4,
...
- Use FeedSilo when the agent needs original captured source text.
- Use GBrain when the agent needs durable synthesized knowledge or a playbook.
- Promote only durable lessons from FeedSilo into GBrain.
- Keep citations back to FeedSilo item IDs and original source URLs.
