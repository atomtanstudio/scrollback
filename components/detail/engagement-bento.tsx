...
export function EngagementBento({
  ...
  return (
    <div className="grid grid-cols-[1fr_1fr] gap-y-1.5 rounded-[16px] border border-[#d6c9b214] bg-[#ffffff08] px-4 py-3">
      {stats.map((stat) => (
        <span key={stat.label} className="flex items-center gap-1.5 text-[13px]">
          <span className="inline-block w-[3.5rem] text-right font-semibold text-[#f2ede5]">{formatNumber(stat.value)}</span>
