...
export function ThreadCard({ item, href }: ThreadCardProps) {
  ...
  return (
    <CardWrapper type="thread" noPadding={hasMedia} href={href}>
      {hasMedia &&
        (() => {
          ...
          return (
            <div className="relative flex h-[180px] w-full items-center justify-center overflow-hidden rounded-t-[23px] bg-[#171d26]">
              ...
              <ThreadPill className="absolute right-3 top-3 bg-[#181d26]/90 backdrop-blur" />
            </div>
          );
        })()}
      ...
      <div className={hasMedia ? "px-5 py-4" : "p-5"}>
        <div className="mb-3 flex items-start justify-between gap-3">
          ...
          {!hasMedia && <ThreadPill className="shrink-0" />}
        </div>
...
function ThreadPill({ className = "" }: { className?: string }) {
  return (
    <div className={`inline-flex items-center gap-1.5 rounded-full border border-[rgba(140,127,159,0.24)] bg-[rgba(140,127,159,0.12)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#c7bad6] ${className}`}>
      <svg
        width="12"
        height="12"
        viewBox="0 0 24 24"
